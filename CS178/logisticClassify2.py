...
## METHODS ################################################################
    ...
    def plotBoundary(self,X,Y):
        ...
        x2b = [(self.theta[0]+self.theta[1]*x1)/(-self.theta[2]) for x1 in x1b]; # find x2 values as a function of x1's values
    ...
    def predict(self, X):
        ...
        for i in range(len(Yhat)):
            Yhat[i] = self.theta[0]+self.theta[1]*X[i,0]+self.theta[2]*X[i,1]
    ...
    def train(self, X, Y, initStep=1.0, stopTol=1e-4, stopEpochs=5000, plot=None):
        ...
        while not done:
            ...
            for i in np.random.permutation(M):
                ri    = XX[i].dot(self.theta.T)     # compute linear response r(x)
                # Yhat = np.sign(ri );
                gradi = self.gradient(XX[i], YY[i],ri);     # compute gradient of NLL loss
